...
import CallingNumberDetailModal from "./CallingNumberDetailModal";
import CallingNumberReAuthModal from "./CallingNumberReAuthModal";
...
export default function CallingNumberTable() {
  ...
  const [selectedItem, setSelectedItem] = useState<CallingNumber | null>(null);
  ...
  return (
    <div className="rounded-md bg-white p-7">
      <DataTable table={table} totalCount={filteredData.length}>
        <CallingNumberTableSearch onSearch={setSearchTerm} />
      </DataTable>

      <CallingNumberDetailModal
        open={detailOpen}
        onOpenChange={setDetailOpen}
        item={selectedItem}
      />

      <CallingNumberReAuthModal
        open={reAuthOpen}
