...
type TemplateMessageType = "sms" | "lms";
type TemplateCategory = "info" | "ad";
...
interface TemplateOption {
  ...
  title: string;
  body: string;
  messageFormat: TemplateMessageType;
...
const templateOptions: TemplateOption[] = [
  {
    ...
    messageFormat: "lms",
    category: "info",
    updatedAt: "2026-04-24 10:12:00",
  },
  {
    id: 2,
    name: "설문 요청 템플릿",
    title: "설문 참여 요청",
    body: "#{이름}님, 설문 참여 부탁드립니다.\n#{설문 링크}",
    messageFormat: "sms",
    category: "info",
    updatedAt: "2026-04-23 14:35:00",
  },
  {
    id: 3,
    name: "프로모션 광고 템플릿",
    title: "봄 시즌 프로모션",
    body: "#{이름}님, #{이벤트명} 혜택 안내드립니다.",
    messageFormat: "lms",
    category: "ad",
    updatedAt: "2026-04-22 09:05:00",
  },
];
...
const normalizeTemplateToken = (content: string) => {
...
export default function SendSmsPage() {
  ...
  const [templateFilter, setTemplateFilter] = useState<"all" | "info" | "ad">(
    "all",
  );
  const [selectedTemplateId, setSelectedTemplateId] = useState<number | null>(
    null,
  );
  const [isTemplateAppliedFromRoute, setIsTemplateAppliedFromRoute] =
    useState(false);
  ...
  }, [templateKeyword, templateFilter]);
  ...
  const handleApplyTemplateBody = () => {
    if (!selectedTemplate) return;
    setMessageBody((prev) =>
      messageType === "ad"
        ? ensureAdDecorators(selectedTemplate.body, rejectNumber)
        : stripAdDecorators(selectedTemplate.body) || prev,
    );
  ...
  const handleApplyTemplateAll = () => {
    if (!selectedTemplate) return;
    const nextMessageType = selectedTemplate.category;
    setMessageTitle(selectedTemplate.title);
    setMessageBody(
      nextMessageType === "ad"
        ? ensureAdDecorators(selectedTemplate.body, rejectNumber)
        : stripAdDecorators(selectedTemplate.body),
    );
    setMessageFormat(selectedTemplate.messageFormat);
  ...
  useEffect(() => {
    if (isTemplateAppliedFromRoute) return;

    const navState = location.state as TemplateNavigationState | null;
    const incomingTemplate = navState?.fromTemplate;
    if (!incomingTemplate) return;

    const nextMessageType: "ad" | "info" =
      incomingTemplate.contentCategory === "AD" ? "ad" : "info";
    const nextMessageFormat: "sms" | "lms" =
      incomingTemplate.messageType === "SMS" ? "sms" : "lms";
    const normalizedBody = normalizeTemplateToken(incomingTemplate.content);

    setMessageTitle(incomingTemplate.name);
    setMessageFormat(nextMessageFormat);
    setMessageType(nextMessageType);
    setMessageBody(
      nextMessageType === "ad"
        ? ensureAdDecorators(normalizedBody, rejectNumber)
        : stripAdDecorators(normalizedBody),
    );
    setIsTemplateAppliedFromRoute(true);
  }, [isTemplateAppliedFromRoute, location.state, rejectNumber]);
  ...
  return (
    <section className="mx-auto w-[1280px] pt-10 pb-[30px]">
          ...
          <section className="rounded-[16px] border border-gray-300 bg-white p-8">
            ...
            <div className="mb-4 flex justify-start">
              <Button
                type="button"
                variant="outline"
                className="h-10 w-[410px] px-4 text-[13px]"
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <FieldBlock label="본문" className="col-span-2">
                ...
                {availableVariables.length > 0 && (
                  <div className="bg-point-gray-100 my-2 rounded-[12px] border border-gray-200 p-4">
      ...
      >
        <DialogContent className="w-[900px] p-0">
          <DialogHeader>
            <DialogTitle>템플릿 불러오기</DialogTitle>
          </DialogHeader>

          <div className="grid grid-cols-[320px_1fr] gap-5 px-8 py-6">
            <div className="space-y-3">
              <Input
                value={templateKeyword}
                onChange={(event) => setTemplateKeyword(event.target.value)}
                placeholder="템플릿명 또는 제목 검색"
                className="h-10"
              />
              <Select
                value={templateFilter}
                onValueChange={(value) =>
                  setTemplateFilter(value as "all" | "info" | "ad")
                }
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="ad">광고성</SelectItem>
                </SelectContent>
              </Select>

              <div className="h-[360px] space-y-2 overflow-y-auto rounded-lg border border-gray-200 p-2">
                {filteredTemplates.map((template) => {
                  ...
                  return (
                    ...
                    >
                      <p className="font-apple-medium text-[13px] text-gray-700">
                        {template.name}
                      </p>
                      <p className="mt-1 text-[12px] text-gray-500">
                        {template.category === "ad" ? "광고성" : "정보성"} ·{" "}
                        {template.messageFormat.toUpperCase()}
                      </p>
                    </button>
                  );
                })}
              ...
              {selectedTemplate ? (
                <div className="mt-3 space-y-3">
                  <div className="rounded-md bg-white p-3">
                    <p className="text-[12px] text-gray-500">템플릿명</p>
                    <p className="font-apple-medium mt-1 text-[14px] text-gray-700">
                      {selectedTemplate.name}
                    </p>
                  </div>
                  <div className="rounded-md bg-white p-3">
                    <p className="text-[12px] text-gray-500">제목</p>
                    <p className="font-apple-medium mt-1 text-[14px] text-gray-700">
                      {selectedTemplate.title}
                    </p>
                  </div>
                  <div className="rounded-md bg-white p-3">
                    <p className="text-[12px] text-gray-500">본문</p>
                    <p className="mt-1 text-[13px] leading-5 whitespace-pre-line text-gray-700">
                      {selectedTemplate.body}
                    </p>
                  </div>
                  <div className="rounded-md bg-white p-3 text-[12px] text-gray-500">
                    {selectedTemplate.category === "ad" ? "광고성" : "정보성"} ·{" "}
                    {selectedTemplate.messageFormat.toUpperCase()} · 수정일{" "}
                    {selectedTemplate.updatedAt}
                  </div>
                </div>
              ) : (
                <div className="mt-3 flex h-[260px] items-center justify-center rounded-md bg-white text-[13px] text-gray-500">
                  적용할 템플릿을 선택해주세요.
                </div>
              )}
            </div>
          ...
          <DialogFooter className="gap-2 px-8 pb-6">
            <Button
              type="button"
              variant="outline"
            ...
            <Button
              ...
              onClick={handleApplyTemplateBody}
            >
